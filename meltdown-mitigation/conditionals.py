...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    plust= threshold+(0.1*threshold)
    mint= threshold-(0.1*threshold)

    if (temperature * neutrons_produced_per_second)<(0.9*threshold):
        return 'LOW'
    elif (temperature * neutrons_produced_per_second)<plust and (temperature * neutrons_produced_per_second)>mint:
        return 'NORMAL'
    else:
        return 'DANGER'
